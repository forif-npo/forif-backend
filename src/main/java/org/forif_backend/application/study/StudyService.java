...
public class StudyService {
     ...
     * @param referenceFiles 참고 자료 파일 정보. Map의 Key는 createStudyApplyRequest 내부에 포함된 Reference의 fileKey와 일치해야 합니다.
     * @return 클라이언트가 S3에 직접 파일을 업로드하는 데 사용할 Presigned URL 정보
     */
    @Transactional
    public CreateStudyApplyResponse createStudyApply(Long MentorId, CreateStudyApplyRequest createStudyApplyRequest, MultipartFile thumbnail, Map<String, MultipartFile> referenceFiles) {
        // 유저 조회
        ...
        studyRepository.saveStudyApply(studyApply);
        studyRepository.saveAllStudyApplyPlan(planList);
        studyRepository.saveAllStudyApplyReference(referenceList);
     ...
     * @return
     */
    private StudyApplyReference toReferenceEntity(CreateStudyApplyRequest.Reference reference, StudyApply studyApply, Map<String, MultipartFile> referenceFiles, List<FileInfo> referenceUploadInfos) {
        String content;
