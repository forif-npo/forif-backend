...
public record UserApplyRequest(
    ...
    @Schema(description = "지원 동기 (정규스터디는 50자 이상 500자 이하, 자율스터디는 미입력)", example = "이 스터디에 지원하는 이유는...", nullable = true)
    @Length(min = 50, max = 500, message = "지원 사유는 50자 이상 500자 이내로 작성해주세요.")
    String applyReason,

    @Schema(description = "지원 순위 (정규스터디: 1 = 1순위, 2 = 2순위 / 자율스터디: 미입력)", example = "1", nullable = true)
