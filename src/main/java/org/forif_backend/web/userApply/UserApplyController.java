...
public class UserApplyController {
    ...
    @Operation(summary = "신청서 상태 변경 (멘토 전용)", description = "신청서의 상태를 승인(ACCEPT), 탈락(REJECT)으로 변경합니다.")
    ...
    @Operation(summary = "합격 처리 (멘토 전용)", description = "지원자 ID 목록을 받아 일괄 합격 처리합니다. 이미 1순위 합격인 지원자는 스킵되고, 2순위 합격 상태에서 1순위 합격 시 2순위 StudyUser가 삭제됩니다.")
    ...
    public ResponseEntity<ApiResponse<Void>> acceptApplications(
        ...
        userApplyService.acceptApplications(userId, studyId, request.applierIds());
        return ResponseEntity.ok(ApiResponse.successWithMsg("Success"));
    }

    @Operation(summary = "불합격 처리 (멘토 전용)", description = "지원서 ID 목록을 받아 일괄 불합격 처리합니다. 이미 불합격인 지원자는 스킵되고, 합격 상태였다면 StudyUser가 삭제됩니다.")
    @PostMapping("/{studyId}/reject")
    public ResponseEntity<ApiResponse<Void>> rejectApplications(
                                                                 @AuthenticationPrincipal Long userId,
                                                                 @Parameter(description = "스터디 ID") @PathVariable("studyId") Integer studyId,
                                                                 @Valid @RequestBody RejectRequest request) {
        userApplyService.rejectApplications(userId, studyId, request.applierIds());
        return ResponseEntity.ok(ApiResponse.successWithMsg("Success"));
    }
}
